...
public class MandelbrotLab {
   ...
   public static double[] multiply(double aReal, double aImag, double bReal, double bImag) {
    ...
    // calculating the real part of the product using (a*c - b*d)
    double realPart = aReal * bReal - aImag * bImag;

    // calculating the imaginary part of the product using (a*d + b*c)
  ...
  public static double magnitudeSquared(double real, double imag) {
    ...
    // calculating the squared magnitude using real^2 + imag^2
   ...
   public static int countIterations(double cReal, double cImag) {
    ...
    // running loop up to MAX_ITERATIONS
    for (int i = 0; i < MAX_ITERATIONS; i++) {

        // calculating z squared using multiply
        double[] zSquared = multiply(zReal, zImag, zReal, zImag);

        // updating z by adding c
        zReal = zSquared[0] + cReal;
        zImag = zSquared[1] + cImag;

        // checking if it escapes (magnitude > 2 --- squared > 4)
...
public static double[] pixelToComplex(int px, int py) {
    ...
    // calculating the real part using the pixel x position
    double cReal = X_MIN + ((double) px / IMAGE_WIDTH) * (X_MAX - X_MIN);

    // calculating the imaginary part using the pixel y position
   ...
   public static int getColor(int iterations) {
    ...
    // if it did not escape, return black
    if (iterations == MAX_ITERATIONS) {
        return 0x000000;
    }

    // calculating grayscale color based on iterations
    int gray = (int)(255.0 * iterations / MAX_ITERATIONS);

    // combining into RGB color
